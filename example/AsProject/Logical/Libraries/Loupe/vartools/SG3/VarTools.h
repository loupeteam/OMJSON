...
#define _VarTools_VERSION 1.0.0
#endif
...
typedef struct varGetAllVars_internal_typ
{	unsigned long iVar;
	unsigned short iDeep;
	unsigned short iMember[50];
	struct variableBrowser Browser;
	plcstring Deep[50][121];
	plcbit NextVariable;
	struct PV_xList_typ xList;
	struct slMoList moList;
	plcstring AppMoName[33];
	plcstring MoName[13];
	plcbit checkGlobal;
	plcbit isGlobal;
	plcbit validVar;
	unsigned long numUniqueVars;
	plcstring uniqueVars[2000][121];
} varGetAllVars_internal_typ;

typedef struct varGetAllVars
{
	/* VAR_OUTPUT (analog) */
	unsigned short Status;
	struct varVariable_typ Variable;
	/* VAR (analog) */
	struct varGetAllVars_internal_typ Internal;
	/* VAR_INPUT (digital) */
	plcbit Execute;
	plcbit AcknowledgeError;
	plcbit PrimitivesOnly;
	plcbit ExpandStructs;
	plcbit CondenseArrays;
} varGetAllVars_typ;
...
_BUR_PUBLIC void varGetAllVars(struct varGetAllVars* inst);
